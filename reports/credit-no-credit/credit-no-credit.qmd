...
Attendance <- DBI::dbGetQuery(CON, "SELECT * FROM attendance") |>
          mutate(check_in = ConvertToLocalPosix(check_in,
                 ...
                 check_out = ConvertToLocalPosix(check_out,
                                                input = 'datetime',
                                                output = 'datetime'),
                 attendance_length = (check_out - check_in) |> as.numeric() |> round(0)
                 )
...
Credit_Threshold <- Attendance |> 
  ...
  summarise(median_attendance = median(attendance_length, na.rm = TRUE) |> round(0)) |> 
  mutate(credit_threshold = round(median_attendance * 0.6, 0))
...
if (any(Result$credit == 0)) {
  ...
  email <- compose_email(
    ...
    footer = md(
      glue::glue(
        ...
        Please do not reply to this email. To edit credit status, please visit
        your [data portal]({Sys.getenv('EDIT_URL_LINK')}).
        "
      )
    )
  )
  ...
  email |> 
    smtp_send(
    # to = c('jwrichey.1@gmail.com', 'corinne.fire@gmail.com', 'sorenson1967@gmail.com', 'Lemondsn85@gmail.com'),
    to = c('jwrichey.1@gmail.com'),
    from = c("noreply@myfirepulse.com" = 'jwrichey.1@gmail.com'),
