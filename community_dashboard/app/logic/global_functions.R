    ...
    for (name in dbTableName) {
      ...
      df <- switch(
        ...
        "firefighter_response" = df |>
          mutate(
            time_adjustment = if_else(is.na(time_adjustment), 0, time_adjustment)
            ),
        df # Default case, no transformation needed
      )
...
ParseUserInput <- function(string) {
  warning('ParseUserInput is deprecated')
